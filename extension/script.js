// TODO: aufräumen, auslagern, refactoring, dokumentieren :-)
...
function start() {
    ...
    calculate = (a, b, c, d, e, f) => {
        ...
        localStorage.setItem('S_aThemen', JSON.stringify(S_aThemen))
    }
    ...
    const loadedS_aThemen = localStorage.getItem('S_aThemen');
    if (loadedS_aThesen && loadedS_aThemen) {

        const startButtonArea = document.getElementsByClassName('intro__action')[0];
        if (startButtonArea) {
            const useSavedAnswersBt = document.createElement("button");
            useSavedAnswersBt.textContent = 'Lade letzte Antworten';
            useSavedAnswersBt.classList += 'button--big';
            useSavedAnswersBt.style = 'margin-right: 1em';
            useSavedAnswersBt.onclick = () => {
                S_aThesen = JSON.parse(loadedS_aThesen);
                S_aThemen = JSON.parse(loadedS_aThemen);
                wom_switch_page(3); // Leite auf die Seite zur Auswahl der Parteien weiter
            }
            startButtonArea.prepend(useSavedAnswersBt);
        }
    }
    ...
    for (let index = 0; index < koalitionen.length; index++) {
        ...
        for (let thesenIndex = 0; thesenIndex < WOMT_nThesen; thesenIndex++) {
            ...
            for (let i = 0; i < koalition.parties.length; i++) {
                ...
                const partyValue = WOMT_aThesenParteien[thesenIndex][partyIndex]
                const partyDegree = 180 - (parseInt(partyValue) + 1) * 90

                WOMT_aThesenParteienText[thesenIndex][koalitionPartyIndex][S_nSprache] += `<p>
                    <span style="font-weight: 700; padding-right: 0.5em">${partyName}</span>
                    <span style="transform: rotate(${partyDegree}deg); display: inline-block;">👍</span>
                    <span style="padding-left: 0.5em">„${WOMT_aThesenParteienText[thesenIndex][partyIndex][S_nSprache]}”</span>
                    </p>`;
            }
        }
...
// Simple timer to make sure everything is ready
